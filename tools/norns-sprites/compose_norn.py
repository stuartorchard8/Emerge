...
AGE="2"; SLOT="v"; HEADFRAME=0; P=9   # pose 9 = side-right; head frame 0 = right-facing
def att(L):
    rows=[]
    ...
    return rows
frames={};atts={}
for L in "abefh": frames[L],_=s16.decode_s16(f"{SRC}/{L}0{AGE}{SLOT}.s16"); atts[L]=att(L)
def bbox(L,fi):
...
ba=att('b')[P]  # body att: head,legL,legR,armL,armR,tail
def assemble(footLdx, footRdx, bodydy):
    placed=[('b',P,0,bodydy)]
    for foot,attpt,dx in (('e',ba[1],footLdx),('h',ba[2],footRdx)):
        fa=atts[foot][P][0]; placed.append((foot,P, attpt[0]-fa[0]+dx, attpt[1]-fa[1]+22+bodydy))
    bx0,by0,bx1,by1=bbox('a',HEADFRAME); hcx=(bx0+bx1)//2; hcy=by1
    placed.append(('a',HEADFRAME, ba[0][0]-hcx, ba[0][1]-hcy+16+bodydy))
    return placed
ORDER='e h b a'.split()
def rasterize(placed):
    ...
    return W,H,cv
specs={"stand":(0,0,0),"walk0":(3,-3,1),"walk1":(-3,3,1)}
rasters={k:rasterize(assemble(*v)) for k,v in specs.items()}
cellW=max(W for W,_,_ in rasters.values())+8; cellH=max(H for _,H,_ in rasters.values())+8
anchorX=cellW//2; anchorY=cellH-3; names=list(specs.keys())
sheetW=cellW*len(names); sheetH=cellH; sheet=[None]*(sheetW*sheetH)
for idx,nm in enumerate(names):
    W,H,cv=rasters[nm]
    xs=[x for y in range(H) for x in range(W) if cv[y*W+x]]; ys=[y for y in range(H) for x in range(W) if cv[y*W+x]]
    cx=(min(xs)+max(xs))//2; fy=max(ys); ox=idx*cellW+anchorX-cx; oy=anchorY-fy
    for y in range(H):
        for x in range(W):
            p=cv[y*W+x]
            if p:
...
json.dump({"breed":"denali","cellW":cellW,"cellH":cellH,"anchorX":anchorX,"anchorY":anchorY,"frames":names}, open(f"{OUT}/denali.json","w"),indent=2)
print(f"sheet {sheetW}x{sheetH} cell {cellW}x{cellH} anchor ({anchorX},{anchorY}) {names}")
