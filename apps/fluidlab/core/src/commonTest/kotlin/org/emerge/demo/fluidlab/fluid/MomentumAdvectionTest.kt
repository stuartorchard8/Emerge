...
import org.emerge.demo.fluidlab.world.fluid.advectMomentum
...
class MomentumAdvectionTest {
    ...
    private class Fluid(val grid: Grid) {
        ...
        val masses = LongArray(grid.size * Species.COUNT)
        ...
        fun step() {
            val tileGrams = tileMass(grid.size, masses)
            val result = advectMass(edges, apertures, MomentumField.of(edges, mx, my), masses, tileGrams)
        ...
        fun put(tile: Int, mass: Long) {
            masses[tile * Species.COUNT + Species.Nitrogen.ordinal] += mass
        }

        val totalGrams: Long get() = masses.sum()
    ...
    fun `momentum travels with the mass that carries it`() {
        ...
        assertTrue(fluid.masses[grid.index(2, 1) * Species.COUNT + Species.Nitrogen.ordinal] > 0L)
    }
    ...
    fun `a face cannot give away more momentum than it holds`() {
        ...
        repeat(25) { tick ->
            ...
            for (g in fluid.masses) assertTrue(g >= 0L, "tick $tick: negative mass")
        }
    }
}
