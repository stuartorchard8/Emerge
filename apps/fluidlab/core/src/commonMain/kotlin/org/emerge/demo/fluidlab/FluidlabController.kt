 ...
 * `FluidlabMain`, `FluidlabAndroidView` and `FluidlabWeb` hosts, none of which knows a rule.
 *
...
class FluidlabController(
    ...
    initial: FluidlabState = FluidlabState.initial(FluidlabConfig()),
) {
    ...
    /** Drains the queued input into one tick's worth of [FluidlabInput]. */
    private fun takeInput(): FluidlabInput {
        if (pendingSpawns.isEmpty() && !pendingClear) return FluidlabInput.EMPTY
        val input = FluidlabInput(spawns = pendingSpawns.toList(), clear = pendingClear)
        pendingSpawns.clear()
        pendingClear = false
        return input
    }
    ...
    fun reset(newState: FluidlabState = FluidlabState.initial(cfg)) {
        pendingSpawns.clear()
        pendingClear = false
        accumulator = 0f
