...
data class Vent(
    ...
    override val energy: Long = ambientJoules(MachineKind.Vent),
) : Machine {
    override val kind: MachineKind get() = MachineKind.Vent
    override fun withWiring(wiring: Wiring): Machine = copy(wiring = wiring)
    override fun withJoules(energy: Long): Machine = copy(energy = energy)
}
...
data class Hull(
    override val wiring: Wiring = Wiring.RUNNING,
    override val energy: Long = ambientJoules(MachineKind.Hull),
) : Machine {
 ...
 * [org.emerge.demo.fluidlab.world.fluid.airlockOpenness] for what that costs and why it is right.
 */
data class Airlock(
    override val wiring: Wiring = SEALED,
    override val energy: Long = ambientJoules(MachineKind.Airlock),
) : Machine {
