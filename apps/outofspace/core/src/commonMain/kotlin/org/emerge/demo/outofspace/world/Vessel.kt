...
/**
 * Fit the grid to this state's bounding box plus [pad] tiles on every side.
 *
 * If the fitted grid matches [state.grid] exactly, returns [state] unchanged; otherwise remaps
 * the world onto the new grid and returns the result. The origin may shift — callers should
 * check camera indices afterwards.
 */
fun VesselState.fitGrid(pad: Int = 4): VesselState {
    ...
    // Diverters
    for (tile in diverters.cursor.keys) {
        val x = grid.xOf(tile)
        val y = grid.yOf(tile)
        if (x < minX) minX = x
        if (y < minY) minY = y
    ...
    // Rocks: positions are in world-space PER_TILE units; convert to tile coords
    for (r in rocks) {
        val tx = (r.positionX / Flight.PER_TILE).toInt()
        val ty = (r.positionY / Flight.PER_TILE).toInt()
        if (tx < minX) minX = tx
        if (ty < minY) minY = ty
        if (tx > maxX) maxX = tx
        if (ty > maxY) maxY = ty
    ...
    // If nothing placed, use the full grid
    val fitMinX = if (maxX < 0) 0 else (minX - pad).coerceAtLeast(0)
    val fitMinY = if (maxY < 0) 0 else (minY - pad).coerceAtLeast(0)
    val fitMaxX = if (maxX < 0) (grid.width - 1) else (maxX + pad).coerceAtMost(grid.width - 1)
    val fitMaxY = if (maxY < 0) (grid.height - 1) else (maxY + pad).coerceAtMost(grid.height - 1)

    val fitWidth = (fitMaxX - fitMinX + 1).coerceAtLeast(1)
    val fitHeight = (fitMaxY - fitMinY + 1).coerceAtLeast(1)

    // If nothing placed at all, keep the original grid
    if (maxX < 0) return this

    // If the fit matches the current grid exactly, no remap needed
    if (fitWidth == grid.width && fitHeight == grid.height && fitMinX == 0 && fitMinY == 0) {
        return this
    }

    // remapped expects dx/dy as "where the old origin lands in the new grid"
    val dx = -fitMinX
    val dy = -fitMinY
    return remapped(Grid(fitWidth, fitHeight), dx, dy)
}
