...
fun gasKelvin(gasEnergy: LongArray, capacity: LongArray): IntArray =
    IntArray(gasEnergy.size) {
        if (capacity[it] <= 0L) Temperature.AMBIENT_KELVIN else (gasEnergy[it] / capacity[it]).toInt()
    }
 ...
 */
fun ambientGasEnergy(tileCount: Int, mass: LongArray): LongArray {
