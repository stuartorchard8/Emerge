 ...
 * `OutofspaceMain`, `OutofspaceAndroidView` and `OutofspaceWeb` hosts, none of which knows a rule.
 *
 * Frame time is *accumulated*, not integrated: the sim only ever advances by whole
 * [OutofspaceConfig.secondsPerTick] steps, so a 144 Hz machine and a 30 Hz phone reach identical
...
class OutofspaceController(
    ...
    initial: OutofspaceState = OutofspaceState.initial(OutofspaceConfig()),
) {
    ...
    val state: OutofspaceState get() = stepper.state
     ...
     */
    fun tick(deltaSeconds: Float, maxTicksPerFrame: Int = 8): OutofspaceState {
        if (!paused) {
    ...
    /** Drains the queued input into one tick's worth of [OutofspaceInput]. */
    private fun takeInput(): OutofspaceInput {
        if (pendingSpawns.isEmpty() && !pendingClear) return OutofspaceInput.EMPTY
        val input = OutofspaceInput(spawns = pendingSpawns.toList(), clear = pendingClear)
        pendingSpawns.clear()
        pendingClear = false
        return input
    }
    ...
    fun reset(newState: OutofspaceState = OutofspaceState.initial(cfg)) {
        pendingSpawns.clear()
        pendingClear = false
        accumulator = 0f
