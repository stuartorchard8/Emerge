...
class ValveTest {
    ...
    private fun plumbed(valveX: Int = 6, y: Int = 6): VesselState {
        var s = VesselState(grid, arrayOfNulls<Machine>(grid.size).toList(), hulled(), buffers = BufferLayer.forMachines(grid, arrayOfNulls<Machine>(grid.size).toList()), rail = RailLayer.empty(grid.size))
        s = pipeRun(s, y, 4, 15)
    ...
    fun `the valve brush lays its own pipe on bare deck and opens one that is already there`() {
        val bare = valveAt(VesselState(grid, arrayOfNulls<Machine>(grid.size).toList(), hulled(), buffers = BufferLayer.forMachines(grid, arrayOfNulls<Machine>(grid.size).toList()), rail = RailLayer.empty(grid.size)), grid.tile(6, 6))
        ...
        var run = pipeRun(VesselState(grid, arrayOfNulls<Machine>(grid.size).toList(), hulled(), buffers = BufferLayer.forMachines(grid, arrayOfNulls<Machine>(grid.size).toList()), rail = RailLayer.empty(grid.size)), 6, 4, 15)
    ...
    fun `a sealed pipe run with no valve on it still takes nothing`() {
        val s = pipeRun(VesselState(grid, arrayOfNulls<Machine>(grid.size).toList(), hulled(), buffers = BufferLayer.forMachines(grid, arrayOfNulls<Machine>(grid.size).toList()), rail = RailLayer.empty(grid.size)), 6, 4, 15)
        val after = run(s, 200)
