...
class ValveTest {
    ...
    fun `the valve brush lays its own pipe on bare deck and opens one that is already there`() {
        val bare = valveAt(VesselState(grid, arrayOfNulls<Machine>(grid.size).toList(), hulled(), buffers = BufferLayer.forMachines(grid, arrayOfNulls<Machine>(grid.size).toList())), grid.tile(6, 6))
        ...
        var run = pipeRun(VesselState(grid, arrayOfNulls<Machine>(grid.size).toList(), hulled(), buffers = BufferLayer.forMachines(grid, arrayOfNulls<Machine>(grid.size).toList())), 6, 4, 15)
    ...
    fun `a sealed pipe run with no valve on it still takes nothing`() {
        val s = pipeRun(VesselState(grid, arrayOfNulls<Machine>(grid.size).toList(), hulled(), buffers = BufferLayer.forMachines(grid, arrayOfNulls<Machine>(grid.size).toList())), 6, 4, 15)
        val after = run(s, 200)
