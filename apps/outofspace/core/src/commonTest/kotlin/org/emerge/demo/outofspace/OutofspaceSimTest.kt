...
class OutofspaceSimTest {
    ...
    private fun run(ticks: Int, seed: Long = 0x5EEDL): OutofspaceState {
        var s = OutofspaceState.initial(cfg, seed = seed)
    ...
    fun `input order does not depend on map iteration order`() {
        val a = mapOf(PlayerId(0) to OutofspaceInput(spawns = listOf(0.1f to 0.1f)), PlayerId(1) to OutofspaceInput(spawns = listOf(-0.2f to 0.3f)))
        val b = mapOf(PlayerId(1) to OutofspaceInput(spawns = listOf(-0.2f to 0.3f)), PlayerId(0) to OutofspaceInput(spawns = listOf(0.1f to 0.1f)))
        val start = OutofspaceState.initial(cfg)
        assertEquals(digest(OutofspaceReducer.reduce(cfg, start, a)), digest(OutofspaceReducer.reduce(cfg, start, b)))
    }

    @Test
    fun `spawns are capped`() {
        val small = cfg.copy(maxBodies = 130)
        var s = OutofspaceState.initial(small)
        repeat(20) {
            s = OutofspaceReducer.reduce(small, s, mapOf(PlayerId(0) to OutofspaceInput(spawns = List(10) { 0f to 0f })))
        }
    ...
    fun `clear empties the world`() {
        val s = OutofspaceReducer.reduce(cfg, OutofspaceState.initial(cfg), mapOf(PlayerId(0) to OutofspaceInput(clear = true)))
