 ...
 */
class TemplateSimTest {

    private val cfg = TemplateConfig()

    /** Digest of the whole world — order-sensitive, so a reordering is a failure too. */
    private fun digest(state: TemplateState): Long {
    ...
    private fun run(ticks: Int, seed: Long = 0x5EEDL): TemplateState {
        var s = TemplateState.initial(cfg, seed = seed)
        repeat(ticks) { s = TemplateReducer.reduce(cfg, s, emptyMap()) }
        return s
    }
    ...
    fun `input order does not depend on map iteration order`() {
        val a = mapOf(PlayerId(0) to TemplateInput(spawns = listOf(0.1f to 0.1f)), PlayerId(1) to TemplateInput(spawns = listOf(-0.2f to 0.3f)))
        val b = mapOf(PlayerId(1) to TemplateInput(spawns = listOf(-0.2f to 0.3f)), PlayerId(0) to TemplateInput(spawns = listOf(0.1f to 0.1f)))
        val start = TemplateState.initial(cfg)
        assertEquals(digest(TemplateReducer.reduce(cfg, start, a)), digest(TemplateReducer.reduce(cfg, start, b)))
    }

    @Test
    fun `spawns are capped`() {
        val small = cfg.copy(maxBodies = 130)
        var s = TemplateState.initial(small)
        repeat(20) {
            s = TemplateReducer.reduce(small, s, mapOf(PlayerId(0) to TemplateInput(spawns = List(10) { 0f to 0f })))
        }
    ...
    fun `clear empties the world`() {
        val s = TemplateReducer.reduce(cfg, TemplateState.initial(cfg), mapOf(PlayerId(0) to TemplateInput(clear = true)))
