...
fun lineageOf(genome: List<Gene>): Lineage {
    ...
    return Lineage(
        ...
        groupNames = genome.map { it.group }.filter { it.isNotEmpty() }.distinct(),
        groupedGeneCount = genome.count { it.group.isNotEmpty() },
        hasRepair = genome.any { it.action.type == ActionType.Repair },
