 ...
 */
object NornsGlView {
    private class State { var paused = false; var stepHz = 10.0 }
    ...
    fun run(seed: Long = 1L) {
        ...
        while (!glfwWindowShouldClose(window)) {
            glfwPollEvents()
            val aspect = updateViewport(window)
            ...
            renderer.draw(world, follow?.x?.toFloat() ?: 0f, aspect, follow?.id)
    ...
    /** Updates the GL viewport to the framebuffer size and returns its aspect (width/height). */
    private fun updateViewport(window: Long): Float {
        MemoryStack.stackPush().use { st ->
            ...
            return fbW.toFloat() / fbH.toFloat()
        }
    }
}
